...
class NN: #Neural Network
    ...
    def countMisorderedPairs(self, patterns):
        ...
        for pair in patterns:
            ...
            if self.propagate(a.features) < self.propagate(b.features):
                winner = b
                loser = a
            else: 
                winner = a
        ...
        p = numMisses / (numRight + numMisses)
        print("     miss: {0}, right: {1}, ratio: {2}".format(numMisses, numRight, p))
        return p
